on start(project, location, panes)
  ...
  tell utils
    ...
    set quaternaryPaneParams to item 4 of panes
    ...
    set quaternaryPaneName    to name of (quaternaryPaneParams & { name: "tertiary" })
    set quaternaryPaneCommand to exec of (quaternaryPaneParams & { exec: "" })
  end tell
  ...
  tell application "iTerm"
    ...
    set quaternaryTerm to (create window with default profile)
    tell quaternaryTerm
      tell current session
        set name to appName & " | " & quaternaryPaneName
        write text "cd " & location
        delay tickTock
        tell utils to clearTerminalScreen()
        if quaternaryPaneCommand is not "" then
          write text quaternaryPaneCommand
        end if
      end tell
    end tell
